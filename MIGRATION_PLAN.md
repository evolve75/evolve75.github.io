...
## Addendum: single-branch consolidation (2026-08-09)

**Status: complete and live.** After the Hugo migration above, the repo was still running the
old two-branch Octopress-era layout: `main` served the built site from repo root, `source` held
the actual Hugo project under a `hugo/` subdirectory, and every deploy meant building locally and
manually copying/committing output into `main`. That workflow was carried over from Octopress
rather than redesigned for Hugo, and it no longer served a purpose once GitHub Pages could build
Hugo itself.

Consolidated to a single `main` branch:

1. Flattened the Hugo project from `hugo/` to the repo root on `main` (content, `hugo.toml`,
   `static/`, `.gitmodules`/theme submodule, `.gitignore`). Files were pulled directly from
   `source`'s git history (`git checkout origin/source -- hugo/...`) rather than the on-disk
   working copy, after discovering the working copy had gone stale/corrupted during earlier
   branch switching.
2. Added `.github/workflows/hugo.yml`, a trimmed version of Hugo's official GitHub Actions
   recipe: builds with the pinned Hugo version and deploys via GitHub Pages' native Actions
   integration (`actions/configure-pages`, `actions/upload-pages-artifact`,
   `actions/deploy-pages`) — no compiled output is ever committed.
3. Switched the repo's Pages `build_type` from `legacy` (branch-serving) to `workflow`
   (Actions-serving) via the GitHub API **before** pushing the flattened content, so the site
   never had a gap between "old branch-served files removed" and "new Actions deploy live."
4. Pushed to `main`, confirmed the Actions run succeeded and the live site (homepage, post,
   archives, feed) served correctly with the expected Hugo generator tag.
5. Tagged the old `source` branch tip as `archive/two-branch-source` (safety net, not meant to
   be developed on) and deleted the `source` branch both remotely and locally.

Net result: `main` now holds only the Hugo source (content, config, theme submodule as a git
submodule). Pushing to `main` is the entire deploy process — no manual build, no copying output
between branches, no second branch to keep in sync.
